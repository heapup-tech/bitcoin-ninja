...
部署 `Runes` 也称作蚀刻 `Etching`, 结构定义在 `Etching` 结构体中
...
另外 `Tag` 枚举中存在 `Flags` 成员, 用于标识 `OP_RETURN` 数据中存储的是什么内容, 值则根据 `Flag` 枚举成员定义。
...
`Flag` 最大值为 127, 最多可以支持 128 种不同的标识。

:::

## 铸造

铸造符文, 则需要设置 `Tag` 枚举中的 `Mint` 成员, 值:
...
符文名称遵循 `Base26` 编码规则, 故名称由大写字母 A-Z 组成。
