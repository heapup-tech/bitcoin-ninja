...
例如在 860000 区块高度部署了符文, 区块内的交易列表序号在 100, 则铸造符文时, 需要构建的 `Key-Value` 数据为:
...
OP_0
OP_IF
  <LE(Base26(RuneName))> // 不含分隔符的符文名称 Base26 编码16进制格式小端序
OP_ENDIF
```
...
铸造符文时由于不需要在 `Witness` 中存放数据, 所以不再需要 `Taproot` 地址。而改用相对较省 Gas 的 `P2WPKH` 地址
...
之后花费发送给 `P2WPKH` 地址的 `UTXO` 并输出包含铸造符文信息的 `OP_RETURN` 类型的 `UTXO`。`OP_RETURN` 数据为
