...
`Runes` 协议, 中文翻译为符文, 由 [Casey Rodarmor](https://x.com/rodarmor) 于比特币第四次减半区块高度 840000 正式发布。不同于 Brc20, `Runes` 协议数据存储在 [OP_RETURN](technical/script/op-return) 中。但 `OP_RETURN` 最多只能承载 80 字节的数据，因此 `Runes` 协议对数据进行压缩编码。

## Runestone

`Runestone` 是一种用于构建和解析比特币交易中 `OP_RETURN` 数据的中间数据结构。
...
- `edicts` - 转账 Runes 数据结构
- `etching` - 部署 Runes 的数据结构
- `mint` - 铸造 Runes 的数据结构
- `pointer` - Runes 分配的到的输出索引

解析交易时提取 `OP_RETURN` 数据, 根据交易的操作, 构建 `Runestone` 结构。

- 转移 Runes 的交易, 解析后的数据赋值给 `edicts` 字段;
- 铸造 Runes 的交易, 解析后的数据赋值给 `mint` 字段;
- 部署 Runes 的交易, 解析后的数据赋值给 `etching` 字段。

## 数据编解码

`OP_RETURN` 中的数据以 **Key-Value** 的形式存储, `Key` 的值如下:
...
- `1: [ 38n ]` 表示 `Divisibility` 的值为 [38n],
- `2: [ 3n ]` 表示 `Flags` 的值为 [3n]
- `3: [ 8256n ]` 表示 `Spacers` 的值为 [8256n]。

在转换成 `OP_RETURN` 数据时, 会依次拼接 `Key` 和 `LEB128` 编码后的 `Value`。
...
如果不进行 `LEB128` 编码, 直接拼接 `Key` 和 `Value`。为了正确读取, 必然还需要存储 `Value` 数组的长度以及每个元素之间的分隔符。这样会增加数据的存储空间, 而 `OP_RETURN` 的存储空间是有限的。
...
部署 Runes 的过程也称作蚀刻 Etching, 用于定义 Runes 的属性和铸造规则。
...
Runes 名称遵循 `Base26` 编码规则, 故名称由大写字母 A-Z 组成。

每个字母从 0 开始, A=0, B=1, ···, Z=25, 再往后 AA = 1 \* 26 + 0 = 26, AB = 1 \* 26 + 1 = 27, 以此类推, 可以覆盖所有的符文名称。
