...
`Reveal` 交易只需要的输入只有该 `UTXO`, 并且提供的解锁脚本在 `witness` 字段中, 以及输出一个包含546聪的新 `UTXO` 给 `Brc20` 数据的所有者。
...
`witness` 字段数据的大小可以确定, 交易输出的大小也可以确定, 整个 `Reveal` 交易的大小便可以确定。根据当前网络费率, 转账金额可以确定。
