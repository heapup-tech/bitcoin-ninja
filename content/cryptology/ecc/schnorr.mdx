...
`Schnorr` 签名基于有限域椭圆曲线, 与 `ECDSA` 类似, 但是 `Schnorr` 相对于 `ECDSA` 来说, 更快速(没有求逆运算), 并且可以实现多签。
...
已知要消息的消息 $m$ 、签名数据 $(R_x, s)$、A 的公钥 $H_A$, B 验证过程如下:
...
`Schnorr` 另一个重要的特征是聚合签名。聚合签名表示将多个签名数据聚合成一个签名数据, 从而减少总签名数据的大小, 降低交易费用。

聚合签名的过程如下:

- 对于每一个签名的参与者使用同一种椭圆曲线和基点, 生成各自的私钥 $k_m$ 和公钥 $P_i$。

- 每个参与者选择不同的随机数 $k_i$, 并计算对应的 $R_i = k_iG$

- 与单签不同的, 聚合签名计算 $e$ 时需要每一个签名参与者的公钥聚合, 以及 $R_i$ 聚合。公钥和 $R_i$ 都是有限域椭圆曲线上的点。聚合最简单的方式点加: 将 $P_i$ 依次相加得到聚合公钥点 $P_{agg} = \sum P_i$, 将 $R_i$ 依次相加得到聚合点 $R_{agg} = \sum R_i$
...
:::warning ECDSA 无法多签聚合的原因

有两个签名: $s_1 = k_1^{-1}(m + r_1k_1) \mod n$, $s_2 = k_2^{-1}(m + r_2k_2) \mod n$

如果直接计算聚合签名: $s_agg = s_1 + s_ 2 = k^{-1}(m + r_1k_1) + k^{-1}(m + r_2k_2) \mod n$

在验证聚合签名 $R_{agg} = s_{agg}^{-1} \dot m \dot G + s\_{agg}^{-1} \dot r_{agg}$
