import { xOnlyPointAddTweak } from '@bitcoinerlab/secp256k1'
...
export function taggedHash(
  ...
  return sha256(
    Uint8Array.from(Buffer.concat([TAGGED_HASH_PREFIXES[prefix], data]))
  )
}
...
export function tapTweakHash(
  ...
  return taggedHash(
    'TapTweak',
    Uint8Array.from(Buffer.concat(h ? [pubKey, h] : [pubKey]))
  )
}

export const tweakKey = (pubKey: Uint8Array, h: Uint8Array | undefined) => {
